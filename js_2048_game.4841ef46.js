...
buttonStart.addEventListener('click', ()=>{
    if (game.getStatus() !== 'idle') game.restart();
    game.start();
    updateView();
...
'use strict';
class Game {
    constructor(size = 4, initialState = [
        [
            0,
            0,
            0,
    ...
    transposeState(state) {
        ...
        for(let col = 0; col < this.size; col++){
            result[col] = [];
            for(let row = 0; row < this.size; row++)result[col].push(state[row][col]);
        }
        return result;
    }
    ...
    checkGameState() {
        for(let row = 0; row < this.size; row++){
            for(let col = 0; col < this.size; col++)if (this.state[row][col] === 2048) {
                this.status = 'win';
                return;
            }
        }
        if (this.hasEmptyTiles() || this.canAdd()) return;
        this.status = 'lose';
    }
    hasEmptyTiles() {
        for(let row = 0; row < this.size; row++)for(let col = 0; col < this.size; col++){
    ...
    canAdd() {
        for(let row = 0; row < this.size; row++)for(let col = 0; col < this.size; col++){
            ...
            if (col < this.size - 1) {
                if (this.state[row][col + 1] === current) return true;
            }
            if (row < this.size - 1) {
                if (this.state[row + 1][col] === current) return true;
            }
        }
        return false;
    }
